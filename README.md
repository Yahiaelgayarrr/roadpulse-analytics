...
The repository includes the compact aggregate outputs generated by the completed
notebook. The dashboard therefore opens with real project results without
requiring the 653 MB raw CSV. Source2-corrected tables are used for comparable
state and annual views; supplied all-source aggregates are clearly labelled as
...
Open `http://127.0.0.1:8050`. The included aggregate tables are sufficient for
all dashboard views.
...
├── data/dashboard/            # Validated real aggregate dashboard tables
